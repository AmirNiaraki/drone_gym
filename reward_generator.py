...
class droneEnv(gym.Env):
	def __init__(self, name):
		...
		if self.name=='cont':
			self.observation_space = Box(low=0, high=255, shape=(FRAME_H, FRAME_W+1), dtype=np.uint8)
...
### Auxiliary functions
	...
	def concat_battery(self, input_frame):#Method receives frame as np array adds a column the end that represent battery level
...
def move_cost(self):#Method to find the step cost based on drag force, for now everything costs 1
		self.cost = 1
